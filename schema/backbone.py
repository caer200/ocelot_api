...
import warnings
from api.routines.geometry import angle_btw, Fitter, unify, get_proj_point2plane
...
class Backbone(MSitelist):
    ...
    def from_omol(cls, omol):
        ...
        for fr in other_fused_rings:
            ...
            if len(set(fr_nbs).intersection(set(backbone_rings_site_idx))) > 0:
                ...
                if angle_btw(fr_vo, vo, output="degree") < 25.0 or angle_btw(-fr_vo, vo, output="degree") < 25.0:
                    backbone_rings += fr
    ...
    def terminate(self):
        """
        only terminate those have nbs different from what they had in omol, this means your cif should be legit
        ...
        for siteid in self.siteids:  # siteid is the id to be terminated
            ...
            if len(omol_nbs_ids) == len(backbon_nbs_ids) + 1:
                ...
                # TODO now we add hydrogens vertical to the plane, it should be sp3 like
