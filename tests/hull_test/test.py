...
def plot_hull(xyzfile):
    omol = MolConformer.from_file(xyzfile)
    name = xyzfile[:-4]
    ref_proj_pts = [get_proj_point2plane(rs.coords, omol.backbone.pfit_vo, omol.backbone.geoc) for rs in omol.backbone.sites]
    ref_2dpts = [coord_transform(omol.backbone.pfit_vp, omol.backbone.pfit_vq, omol.backbone.pfit_vo, p3d)[:2] for p3d in ref_proj_pts]
