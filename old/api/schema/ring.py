...
class Ring(MSitelist):
    ...
    def __init__(self, sites, omol):
        super().__init__(sites)
        self.omol = omol
        self.number = len(self.sites)
        self.idx = [s.siteid for s in self.sites]
        if self.number < 3:
            sys.exit('you are initializing a ring with less than 3 msites!')

    @property
    def normals(self):
        coordmat = self.omol.coordmat[[s.siteid for s in self.sites]]
    ...
    def bonds(self):
        bonds = []
        for si in self.idx:
            for j in self.omol.nbrmap[si]:
                b = Bond([self.omol.sites[si], self.omol.sites[j]])
                if b not in bonds:
                    bonds.append(b)
        return bonds
