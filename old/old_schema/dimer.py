from api.routines.geometry import angle_btw, coord_transform, get_proj_point2plane, alpha_shape, norm
...
class Dimer:
    ...
    def __init__(self, omol_ref, omol_var, config, label=""):
        ...
        self.pslip = abs(self.vslip @ ref_bone.vp)
        self.qslip = abs(self.vslip @ ref_bone.vq)
        self.oslip = abs(self.vslip @ ref_bone.vo)
        self.pangle = angle_btw(ref_bone.vp, var_bone.vp, output='degree')
        self.qangle = angle_btw(ref_bone.vq, var_bone.vq, output='degree')
        self.oangle = angle_btw(ref_bone.vo, var_bone.vo, output='degree')
        if self.oangle > 90:
    ...
    def minbonedist(self):
        distmat = cdist(self.omol_ref.backbone.get_coordmat(), self.omol_var.backbone.get_coordmat())
...
class DimerCollection:
    ...
    def get_xyz_string(self):
        ...
        for d in self.dimers:
            sites += [s.to_pymatgen_site() for s in d.omol_var.sites]
        sites += [Site('La', ss.coords) for ss in self.dimers[0].omol_ref.sites]
