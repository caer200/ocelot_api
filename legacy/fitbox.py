...
from ocelot.routines.geometry import rotate_along_axis, unify, rotation_matrix, norm, angle_btw
"""
conformation gen with fixed pbc

this is useful for solving gixd structures
"""
#TODO current version is very slow, try to apply something like
# https://bmcchembiol.biomedcentral.com/articles/10.1186/1472-6769-9-6
...
# class Boxfitter:
#
#     def __init__(self, omol, pbc):
#         """
#         class for conformation gen
#
#         :param omol: OMol object
#         :param pbc: 3x3 np array
#         """
#         self.omol = deepcopy(omol)
#         self.pbc = np.array(pbc)
#         a, b, c = self.pbc
#         self.transvs = [a, b, c, a - b, a - c, b - c, a + b, a + c, b + c]
#
#     def tooclose(self, conform, cutoff=3):
#         """
#         decide if the config generated is too close to any of its images
#
#         :param conform:
#         :param cutoff: in \AA
#         :return: bool
#         """
#         coordmat = conform.get_coordmat()
...
#         return False
#
#     def gen_bone_conform(self, steps=180):
#         original_backbone = self.omol.backbone
#         backbone = Backbone.orient(original_backbone, pqo=original_backbone.pqo)  # deepcopy, geoc=origin vp=x vq=y vo=z
#         bone_configs = []
...
#             bone_configs.append(MSitelist.from_coordmat(coords, names, ids))
...
#             vcoord = np.matmul(v, self.omol.backbone.pqomat.T)
...
#         current_site = None
#         for s in config:
#             s_origin = self.omol.get_site(s.id)
...
#         else:
#             configs = []
#             current_site_in_mol = self.omol.get_site(current_site.id)
...
#         if len(c) == 0:
#             return c
#         c = [self.addsj(bone_config, self.omol.sidechains, bone_pqo)]
